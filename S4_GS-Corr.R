       ...
       ##the GS-Corr page      
         ...
         ")
                     ...
                     tabPanel("Gene Set Membership",
                              HTML("<p>This tab shows which gene sets the selected gene belongs to. If no data is available in the table then this gene is not in any gene sets.</p> 
         ")
                              ,
 ...
 ##download genes in pathway
  ...
  gsmem<-eventReactive(
    eventExpr = input$run_membership, {
      ...
      colnames(paths_to_fill)<-"Pathways containing gene"
      paths_to_fill<-paths_to_fill%>%
        filter(!`Pathways containing gene`=="not present")
